...
node_editor_init(struct node_editor *editor)
{
    ...
    editor->begin = NULL;
    editor->end = NULL;
    node_editor_add(editor, "Source", nk_rect(40, 10, 180, 220), nk_rgb(255, 0, 0), 0, 2);
    node_editor_add(editor, "Source", nk_rect(40, 260, 180, 220), nk_rgb(0, 255, 0), 0, 2);
    node_editor_add(editor, "Combine", nk_rect(400, 100, 180, 220), nk_rgb(0,0,255), 20, 2);
/*    for(int i=0;i<1000;i++){
      node_editor_add(editor, "Combine", nk_rect(400, 100+220*i, 180, 220), nk_rgb(0,0,255), 2, 2);
    }*/
    ...
    if (nk_begin(ctx, "NodeEdit", nk_rect(0, 0, 800, 600),
        ...
        nk_layout_space_begin(ctx, NK_STATIC, total_space.h, nodedit->node_count);
        {
            ...
            while (it) {
                ...
                /* calculate hight*/
                it->bounds.h = 25+((it->input_count>it->output_count)? 25*it->input_count:25*it->output_count);
...
//                      nk_text(ctx, "0", 15, NK_TEXT_RIGHT);
                ...
                }
                {
                    ...
                    for (n = 0; n < it->output_count; ++n) {
                        ...
                        struct nk_rect text_descript;
                        circle.x = node->bounds.x + node->bounds.w-4;
                        text_descript.x = circle.x - 10;
                        circle.y = node->bounds.y + space * (float)(n+1);
                        text_descript.y =circle.y;
                        circle.w = 8; circle.h = 8;
                        text_descript.w = 10;text_descript.h=10;
                        nk_fill_circle(canvas, circle, nk_rgb(100, 100, 100));
                        const struct nk_user_font *f = ctx->style.font;
                        nk_draw_text(canvas, text_descript, "1", 1, f, nk_rgb(120, 100, 100), nk_rgb(20, 100, 100));
//                        nk_propertyi(ctx, "out", 0, 4, 25, 1,1);
                    ...
                    for (n = 0; n < it->input_count; ++n) {
                        ...
                        nk_fill_circle(canvas, circle, nk_rgb(100, 100, 100));
                        const struct nk_user_font *f = ctx->style.font;
                        nk_itoa(number_buffer, n);
                        num_len = nk_strlen(number_buffer);
                        nk_draw_text(canvas, text_descript, number_buffer, num_len, f, nk_rgb(120, 100, 100), nk_rgb(20, 100, 100));
            ...
            for (n = 0; n < nodedit->link_count; ++n) {
                ...
                float spaceo = no->bounds.h / (float)((no->input_count) + 1);
            ...
            if (nk_contextual_begin(ctx, 0, nk_vec2(100, 220), nk_window_get_bounds(ctx))) {
                ...
                if (nk_contextual_item_label(ctx, "New", NK_TEXT_CENTERED)){
                    ...
                    node_editor_add(nodedit, "New", nk_rect(400, 260, 180, 220),
                            nk_rgb(255, 255, 255), input_cnt, 2);
                    input_cnt++;
                }
