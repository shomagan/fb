...
void fb00077_exec (void)
{
	...
	if (IN->CmdARX.Data.bit) { // Если архивирование замера разрешено
		...
		// Время начала периода измерения
    OUT->ArcArray[0].Data.uint8 = IN->StartOtvNum.Data.uint8;
    OUT->ArcArray[1].Data.uint8 = IN->EndOtvNum.Data.uint8;
    OUT->ArcArray[2].Data.uint8 = (char) (IN->StartTime.Data.uint32 & 0xFF);    // 11
    OUT->ArcArray[3].Data.uint8 = (char) (IN->StartTime.Data.uint32>>8);        // 12
    OUT->ArcArray[4].Data.uint8 = (char) (IN->StartTime.Data.uint32>>16);       // 13
    OUT->ArcArray[5].Data.uint8 = (char) (IN->StartTime.Data.uint32>>24);       // 14
    ...
    for (i = 6;i<54;i++){
      OUT->ArcArray[i].Data.uint8 = R7Buff[i];
    }
